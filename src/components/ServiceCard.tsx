...
export function ServiceCard({ icon: Icon, title, description }: ServiceCardProps) {
  return (
    <motion.article
      ...
      whileHover={{ y: -7 }}
      className="group soft-surface relative h-full overflow-hidden rounded-4xl p-6 shadow-soft transition-all duration-300 hover:shadow-[0_20px_40px_rgba(27,26,26,0.12)] sm:p-8"
    >
      ...
      <p className="mt-3 text-sm leading-relaxed text-slateWarm sm:text-base">{description}</p>
    </motion.article>
  );
}
